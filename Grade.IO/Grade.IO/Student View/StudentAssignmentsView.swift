...
class StudentAssignmentsView: UIViewController {
    ...
    lazy var scrollView: UIScrollView! = {
        let view = UIScrollView(frame: CGRect(x: 0, y: (studentGrade.frame.origin.y)  + (studentGrade.frame.height) + 30, width: self.view.frame.width, height: self.view.frame.height - studentGrade.frame.height))
