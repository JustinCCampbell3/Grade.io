...
class TeacherAssignmentPage: UIViewController, MenuControllerDelegate {
    ...
    lazy var scrollView: UIScrollView! = {
        let view = UIScrollView(frame: CGRect(x: 0, y: (addAssignBtn.frame.origin.y * 2)  + (addAssignBtn.frame.height), width: self.view.frame.width, height: self.view.frame.height - addAssignBtn.frame.height))
